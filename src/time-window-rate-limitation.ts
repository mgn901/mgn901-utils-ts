...
const calculateNextExecutionDateOfEachRule = async (params: {
  ...
  readonly getNewestExecutionDateInLatestTimeWindow: (
    this: unknown,
  ) => Date | Promise<Date>;
  ...
  return (
    await Promise.all(
      params.timeWindowRateLimitationRules.map(async (rule) => {
        ...
        const startOfLatestTimeWindow = new Date(
          endOfLatestTimeWindow - rule.timeWindowMs,
        );

        const executionCountInLatestTimeWindow =
          await params.countExecutionsInLatestTimeWindow(
            startOfLatestTimeWindow,
          );

        const oldestExecutionInLatestTimeWindow =
          await params.getOldestExecutionDateInLatestTimeWindow(
            startOfLatestTimeWindow,
          );
        ...
        const nextExecutionDateInLatestTimeWindow =
          oldestExecutionDateInLatestTimeWindow +
          intervalMs * executionCountInLatestTimeWindow;

        return {
          rule,
          nextExecutionDate: new Date(
            Math.max(Date.now(), nextExecutionDateInLatestTimeWindow),
          ),
          isExceeded:
            rule.executionCountPerTimeWindow <=
            executionCountInLatestTimeWindow,
        };
      }),
    )
  ).sort(
    (a, b) => a.nextExecutionDate.getTime() - b.nextExecutionDate.getTime(),
  );
};
...
export const calculateNextExecutionDate = async (params: {
  ...
  const nextExecutionDateOfEachRule =
    await calculateNextExecutionDateOfEachRule(params);
